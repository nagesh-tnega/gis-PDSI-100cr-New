<?php
	...
	if (isset($_POST['case']) || isset($_GET['case']))
	{
		...
		switch ($case)
		{
           ...
           case 'downloadProjectReport':
			...
			echo json_encode($projectFilter);
            break;
			default:
            //echo $_POST['case'];
			break;
	...
	function getDept()
    {
        ...
        $query = pg_query(DBCON, "select distinct a.hod_name  as hod_cod,b.hod_name as hod_name from sp_index_v2 as a, masters.dept_users as b where a.hod_name = b.hod_dept_code order by b.hod_name asc");
	...
	function getReport() {
		 ...
		 $result = pg_query(DBCON, "select a.dept_name,a.dept_code, a.hod_name,a.hod_dept_code, b.name_of_the_project, b.funding_agency, b.short_description_of_the_project, b.project_start_date, b.project_end_date, b.project_cost, b.fund_received, b.fund_yet_to_be_received,b.remarks,b.eot, b.id,b.layers, b.latitude, b.longitude, b.causes_of_delay,b.cost_overrun_reasons,b.cost_overrun,DATE(b.updated_time) as updated_date,b.district_name,b.taluk_name,b.village_name,b.project_id from masters.dept_users as a , sp_index_v2 as b where a.hod_dept_code = b.hod_name and a.dept_name != '' order by a.dept_name asc");
  ...
  function getGeomCol($table) {
	...
	function getProjectAbstract(){
		if(isset($_POST['dId']) && isset($_POST['hId']) && isset($_POST['pId'])){
			...
			if($dept_code == "ALL"){
				$where = "";
			}else if($dept_code != "ALL" && $hod_code == "all_hod"){
				$where = " where sp.hod_name like '$dept_code%'";
			}else if($dept_code != "ALL" && $hod_code != "all_hod" && $project_id == "ALL"){
	...
	function downloadAbstractReport(){
		if(isset($_POST['dId']) && isset($_POST['hId']) && isset($_POST['pId'])){
			...
			$query = "select sp.department_name,d.hod_name,sp.implementing_agency,sp.project_id,name_of_the_project as project_name,sp.funding_agency,sp.government_orders,sp.short_description_of_the_project as project_desc,sp.project_start_date,sp.project_end_date,sp.project_cost,sp.district_name,sp.taluk_name,sp.village_name,sp.remarks,cost_overrun,cost_overrun_reasons,causes_of_delay from sp_index_v2 as sp inner join masters.dept_users as d on sp.hod_name=d.hod_dept_code" . $where . " order by sp.department_name,sp.implementing_agency,sp.name_of_the_project";
			$result = pg_query(DBCON, $query);
			$rs = pg_fetch_all($result);
			if($rs){
	...
	function projects(){
		...
		$hod_code = $_POST['hod'];
		if ($dept_code == 'all' && $hod_code == 'all_hod'){
			$query = "select * from sp_index_v2 where dept_code != '' and hod_name != '' and created_time IS NOT NULL order by name_of_the_project asc";
		}elseif($dept_code == 'all' && $hod_code != 'all_hod'){
			$query = "select * from sp_index_v2 where dept_code != '' and hod_name = '$hod_code' and created_time IS NOT NULL order by name_of_the_project asc";
		}elseif($dept_code != 'all' && $hod_code == 'all_hod'){
			$query = "select * from sp_index_v2 where dept_code = '$dept_code' and hod_name != '' and created_time IS NOT NULL order by name_of_the_project asc";
		}
		else {
			$query = "select * from sp_index_v2 where dept_code = '$dept_code' and hod_name = '$hod_code' and created_time IS NOT NULL order by name_of_the_project asc";
		}
		$result = pg_query($dbacon, $query);
		$count = pg_num_rows($result);
    	if ($count > 0) {
    		...
    		while ($row =  pg_fetch_assoc($result)) {
    			$html .= '<option value="'.$row['id'].'">'.$row['name_of_the_project'].'</option>';
	        }
    	}else{
    ...
    function departments(){
        ...
        while ($rs = pg_fetch_array($query))
        {
            ...
            $html .= '<option value="' . $rs['dept_code'] . '">'.$rs['short_name'].' ('.$count_1.' Projects)</option>';
		}
    ...
    function project_details(){
		...
		$query = "SELECT project_id, department_name, implementing_agency, name_of_the_project, funding_agency, government_orders, short_description_of_the_project, project_start_date, project_end_date, project_cost, fund_received, fund_yet_to_be_received, district_name, taluk_name, village_name, survey_no, latitude, longitude, total_project_extend, work_completed_details_with_extend, work_in_progress_details_with_extend, work_to_be_completed_details_with_extend, photo_prior_to_commencement_of_work, photo_current_status, remarks, layers, geom, hod_name, DATE(created_time) as created_time, DATE(updated_time) as updated_time, id, eot, revised_start_date, revised_end_date, present_status, causes_of_delay, other_reasons_delay, cost_overrun, cost_overrun_reasons, dept_code FROM sp_index_v2 where id = '$proj_id'";
		$result = pg_query($dbacon, $query);
		$count = pg_num_rows($result);
    	if ($count > 0) {
    		while ($row =  pg_fetch_assoc($result)) {
    ...
    function project_search(){
    	$dbacon = DBCON;
		// $search = $_POST['search'];
		$query = "select project_id,name_of_the_project,id from sp_index_v2 where created_time IS NOT NULL order by name_of_the_project asc";
		$result = pg_query($dbacon, $query);
		$count = pg_num_rows($result);
    ...
    function hods(){
		...
		$count = pg_num_rows($result);
    	if ($count > 0) {
    		$html = '<option value="null" selected>Select HOD</option>';
    		$html .= '<option value="all_hod">All HODs</option>';
    		while ($row =  pg_fetch_assoc($result)) {
    			$code = $row['hod_dept_code'];
    			$query_1 = "select * from sp_index_v2 where hod_name = '$code' and created_time IS NOT NULL order by name_of_the_project asc";
    			$result_1 = pg_query($dbacon, $query_1);
				$count_1 = pg_num_rows($result_1);
    			$html .= '<option value="'.$row['hod_dept_code'].'">'.$row['hod_name'].' ('.$count_1.' Projects)</option>';
	        }
    	}else{
	...
	function projectFilter() {
		...
		$data_type_list = $_POST['data_type_list'];
		if($list_type == "all"){
			if($data_type_list == "all_data"){
				$sql = "select row_number() over (order by a.dept_code asc) AS sno, a.dept_name,a.dept_code, a.hod_name,a.hod_dept_code, b.name_of_the_project, b.funding_agency, b.short_description_of_the_project, b.project_start_date, b.project_end_date, b.project_cost, b.fund_received, b.fund_yet_to_be_received,b.remarks,b.eot, b.id,b.layers, b.latitude, b.longitude, b.causes_of_delay,b.cost_overrun_reasons,b.cost_overrun,DATE(b.updated_time) as updated_date,b.district_name,b.taluk_name,b.village_name,b.project_id,b.created_time from masters.dept_users as a , sp_index_v2 as b where a.hod_dept_code = b.hod_name and a.dept_name != '' and b.dept_code!='' order by a.dept_name asc";
			}
		}else if($list_type == "data_avail"){
			if($data_type_list == "mis_gis"){
				$sql = "select a.dept_name,a.dept_code, a.hod_name,a.hod_dept_code, b.name_of_the_project, b.funding_agency, b.short_description_of_the_project, b.project_start_date, b.project_end_date, b.project_cost, b.fund_received, b.fund_yet_to_be_received,b.remarks,b.eot, b.id,b.layers, b.latitude, b.longitude, b.causes_of_delay,b.cost_overrun_reasons,b.cost_overrun,DATE(b.updated_time) as updated_date,b.district_name,b.taluk_name,b.village_name,b.project_id,b.created_time,b.dept_code from masters.dept_users as a , sp_index_v2 as b where a.hod_dept_code = b.hod_name and a.dept_name != '' and b.layers !='' and b.layers IS NOT NULL and b.created_time IS NOT NULL and b.dept_code!='' order by a.dept_name asc";
			}elseif($data_type_list == "mis"){
				$sql = "select a.dept_name,a.dept_code, a.hod_name,a.hod_dept_code, b.name_of_the_project, b.funding_agency, b.short_description_of_the_project, b.project_start_date, b.project_end_date, b.project_cost, b.fund_received, b.fund_yet_to_be_received,b.remarks,b.eot, b.id,b.layers, b.latitude, b.longitude, b.causes_of_delay,b.cost_overrun_reasons,b.cost_overrun,DATE(b.updated_time) as updated_date,b.district_name,b.taluk_name,b.village_name,b.project_id,b.created_time,b.dept_code from masters.dept_users as a , sp_index_v2 as b where a.hod_dept_code = b.hod_name and a.dept_name != '' and (b.layers = '' OR b.layers IS NULL) and (b.created_time IS NOT NULL and b.dept_code!='') order by a.dept_name asc";
			}elseif($data_type_list =="no_data"){
				$sql = "select a.dept_name,a.dept_code, a.hod_name,a.hod_dept_code, b.name_of_the_project, b.funding_agency, b.short_description_of_the_project, b.project_start_date, b.project_end_date, b.project_cost, b.fund_received, b.fund_yet_to_be_received,b.remarks,b.eot, b.id,b.layers, b.latitude, b.longitude, b.causes_of_delay,b.cost_overrun_reasons,b.cost_overrun,DATE(b.updated_time) as updated_date,b.district_name,b.taluk_name,b.village_name,b.project_id,b.created_time,b.dept_code from masters.dept_users as a , sp_index_v2 as b where a.hod_dept_code = b.hod_name and a.dept_name != '' and (b.layers = '' OR b.layers IS NULL) and (b.created_time IS NULL and b.dept_code!='') order by a.dept_name asc";
			}else{
				$sql = "select row_number() over (order by a.dept_code asc) AS sno, a.dept_name,a.dept_code, a.hod_name,a.hod_dept_code, b.name_of_the_project, b.funding_agency, b.short_description_of_the_project, b.project_start_date, b.project_end_date, b.project_cost, b.fund_received, b.fund_yet_to_be_received,b.remarks,b.eot, b.id,b.layers, b.latitude, b.longitude, b.causes_of_delay,b.cost_overrun_reasons,b.cost_overrun,DATE(b.updated_time) as updated_date,b.district_name,b.taluk_name,b.village_name,b.project_id,b.created_time,b.dept_code from masters.dept_users as a , sp_index_v2 as b where a.hod_dept_code = b.hod_name and a.dept_name != '' and b.dept_code!='' order by a.dept_name asc";
			}
		}else if($list_type == "time_overrun"){
			if($data_type_list == "completed"){
				$sql = "select a.dept_name,a.dept_code, a.hod_name,a.hod_dept_code, b.name_of_the_project, b.funding_agency, b.short_description_of_the_project, b.project_start_date, b.project_end_date, b.project_cost, b.fund_received, b.fund_yet_to_be_received,b.remarks,b.eot, b.id,b.layers, b.latitude, b.longitude, b.causes_of_delay,b.cost_overrun_reasons,b.cost_overrun,DATE(b.updated_time) as updated_date,b.district_name,b.taluk_name,b.village_name,b.project_id,b.created_time,b.dept_code from masters.dept_users as a , sp_index_v2 as b where a.hod_dept_code = b.hod_name and a.dept_name != '' and b.present_status like '%Completed%' and b.dept_code!='' order by a.dept_name asc";
			}else if($data_type_list == "in_progress"){
				$sql = "select a.dept_name,a.dept_code, a.hod_name,a.hod_dept_code, b.name_of_the_project, b.funding_agency, b.short_description_of_the_project, b.project_start_date, b.project_end_date, b.project_cost, b.fund_received, b.fund_yet_to_be_received,b.remarks,b.eot, b.id,b.layers, b.latitude, b.longitude, b.causes_of_delay,b.cost_overrun_reasons,b.cost_overrun,DATE(b.updated_time) as updated_date,b.district_name,b.taluk_name,b.village_name,b.project_id,b.created_time,b.dept_code from masters.dept_users as a , sp_index_v2 as b where a.hod_dept_code = b.hod_name and a.dept_name != '' and b.present_status = 'In progress' and b.dept_code!='' order by a.dept_name asc";
			}else if($data_type_list == "delay"){
				$sql = "select a.dept_name,a.dept_code, a.hod_name,a.hod_dept_code, b.name_of_the_project, b.funding_agency, b.short_description_of_the_project, b.project_start_date, b.project_end_date, b.project_cost, b.fund_received, b.fund_yet_to_be_received,b.remarks,b.eot, b.id,b.layers, b.latitude, b.longitude, b.causes_of_delay,b.cost_overrun_reasons,b.cost_overrun,DATE(b.updated_time) as updated_date,b.district_name,b.taluk_name,b.village_name,b.project_id,b.created_time,b.dept_code from masters.dept_users as a , sp_index_v2 as b where a.hod_dept_code = b.hod_name and a.dept_name != '' and  (b.present_status = '' OR b.present_status IS NULL OR b.present_status = 'Yet to be started') and b.dept_code!='' order by a.dept_name asc";
			}else{
				$sql = "select row_number() over (order by a.dept_code asc) AS sno, a.dept_name,a.dept_code, a.hod_name,a.hod_dept_code, b.name_of_the_project, b.funding_agency, b.short_description_of_the_project, b.project_start_date, b.project_end_date, b.project_cost, b.fund_received, b.fund_yet_to_be_received,b.remarks,b.eot, b.id,b.layers, b.latitude, b.longitude, b.causes_of_delay,b.cost_overrun_reasons,b.cost_overrun,DATE(b.updated_time) as updated_date,b.district_name,b.taluk_name,b.village_name,b.project_id,b.created_time,b.dept_code,b.present_status from masters.dept_users as a , sp_index_v2 as b where a.hod_dept_code = b.hod_name and a.dept_name != '' and b.dept_code!='' order by a.dept_name asc";
			}
		}else if($list_type == "data_update"){
			if($data_type_list == "less1"){
				$sql = "select a.dept_name,a.dept_code, a.hod_name,a.hod_dept_code, b.name_of_the_project, b.funding_agency, b.short_description_of_the_project, b.project_start_date, b.project_end_date, b.project_cost, b.fund_received, b.fund_yet_to_be_received,b.remarks,b.eot, b.id,b.layers, b.latitude, b.longitude, b.causes_of_delay,b.cost_overrun_reasons,b.cost_overrun,DATE(b.updated_time) as updated_date,b.district_name,b.taluk_name,b.village_name,b.project_id,b.created_time,b.dept_code from masters.dept_users as a , sp_index_v2 as b where a.hod_dept_code = b.hod_name and a.dept_name != '' and b.updated_time = date_trunc('month', current_date) and b.dept_code!='' order by a.dept_name asc";
			}else if($data_type_list == "oneto3"){
				$sql = "select a.dept_name,a.dept_code, a.hod_name,a.hod_dept_code, b.name_of_the_project, b.funding_agency, b.short_description_of_the_project, b.project_start_date, b.project_end_date, b.project_cost, b.fund_received, b.fund_yet_to_be_received,b.remarks,b.eot, b.id,b.layers, b.latitude, b.longitude, b.causes_of_delay,b.cost_overrun_reasons,b.cost_overrun,DATE(b.updated_time) as updated_date,b.district_name,b.taluk_name,b.village_name,b.project_id,b.created_time,b.dept_code from masters.dept_users as a , sp_index_v2 as b where a.hod_dept_code = b.hod_name and a.dept_name != '' and b.updated_time >= date_trunc('month', current_date-interval '3' month) AND b.updated_time < date_trunc('month', current_date) and b.dept_code!='' order by a.dept_name asc";
			}else if($data_type_list == "less3"){
				$sql = "select a.dept_name,a.dept_code, a.hod_name,a.hod_dept_code, b.name_of_the_project, b.funding_agency, b.short_description_of_the_project, b.project_start_date, b.project_end_date, b.project_cost, b.fund_received, b.fund_yet_to_be_received,b.remarks,b.eot, b.id,b.layers, b.latitude, b.longitude, b.causes_of_delay,b.cost_overrun_reasons,b.cost_overrun,DATE(b.updated_time) as updated_date,b.district_name,b.taluk_name,b.village_name,b.project_id,b.created_time,b.dept_code from masters.dept_users as a , sp_index_v2 as b where a.hod_dept_code = b.hod_name and a.dept_name != '' and (b.updated_time < date_trunc('month', current_date-interval '3' month) OR b.updated_time IS NULL) and b.dept_code!='' order by a.dept_name asc";
			}else{
				$sql = "select row_number() over (order by a.dept_code asc) AS sno, a.dept_name,a.dept_code, a.hod_name,a.hod_dept_code, b.name_of_the_project, b.funding_agency, b.short_description_of_the_project, b.project_start_date, b.project_end_date, b.project_cost, b.fund_received, b.fund_yet_to_be_received,b.remarks,b.eot, b.id,b.layers, b.latitude, b.longitude, b.causes_of_delay,b.cost_overrun_reasons,b.cost_overrun,DATE(b.updated_time) as updated_date,b.district_name,b.taluk_name,b.village_name,b.project_id,b.created_time,b.dept_code from masters.dept_users as a , sp_index_v2 as b where a.hod_dept_code = b.hod_name and a.dept_name != '' and b.dept_code!='' order by a.dept_name asc";
			}
		}
